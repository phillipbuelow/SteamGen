...
function solvedTdt(T, t, n, p, sh, rh, ev, ph, d)
    ...
    dTdt(2:n.N_s_p) = (p.h_s(2:end) .* p.A_s .* (T_mw_p(2:end) - T_s_p(2:end)) + sh.mdot_s(tt) .* p.cp_s(1:end-1) .* T_s_p(1:end-1) - sh.mdot_s(tt) .* p.cp_s(2:end) .* T_s_p(2:end)) ./ (p.rho_s(2:end) .* p.cp_s(2:end) .* p.V_s);
    ...
    dTdt(n.N_s_p+1) = ((T_s_p(1) - T_mw_p(1)) ./ (1 ./ (p.h_s(1) * p.A_s) + log(p.r_c_p / p.r_i_p) / (2 * pi * p.dx * p.k_mw(1))) ...
        ...
        + (T_mw_p(2) - T_mw_p(1)) * p.k_mw(1) * p.A_c_mw / p.dx) ...
        ./ (p.rho_mw(1) * p.cp_mw(1) * p.V_mw);
    dTdt(n.N_s_p+2:n.N_mw_p-1) = ((T_s_p(2:end-1) - T_mw_p(2:end-1)) ./ (1 ./ (p.h_s(2:end-1) * p.A_s) + log(p.r_c_p / p.r_i_p) / (2 * pi * p.dx * p.k_mw(2:end-1))) ...
        ...
        + (T_mw_p(3:end) - T_mw_p(2:end-1)) * p.k_mw(2:end-1) * p.A_c_mw / p.dx) ...
        ./ (p.rho_mw(2:end-1) * p.cp_mw(2:end-1) * p.V_mw);
    dTdt(n.N_mw_p) = ((T_s_p(end) - T_mw_p(end)) ./ (1 ./ (p.h_s(end) * p.A_s) + log(p.r_c_p / p.r_i_p) / (2 * pi * p.dx * p.k_mw(end))) ...
        ...
        + (T_mw_p(end-1) - T_mw_p(end)) * p.k_mw(end) * p.A_c_mw / p.dx) ...
        ./ (p.rho_mw(end) * p.cp_mw(end) * p.V_mw);
    ...
    dTdt(n.N_s_sh+1) = ((T_s_sh(1) - T_m_sh(1)) ./ (1 ./ (sh.h_s(1) * sh.A_s) + log(sh.r_c_sh / sh.r_i_sh) / (2 * pi * sh.dx * sh.k_mw(1))) ...
        ...
        + (T_m_sh(2) - T_m_sh(1)) * sh.k_mw(1) * sh.A_c_mw / sh.dx) ...
        ./ (sh.rho_mw(1) * sh.cp_mw(1) * sh.V_mw);
    dTdt(n.N_s_sh+2:n.N_m_sh-1) = ((T_s_sh(2:end-1) - T_m_sh(2:end-1)) ./ (1 ./ (sh.h_s(2:end-1) * sh.A_s) + log(sh.r_c_sh / sh.r_i_sh) / (2 * pi * sh.dx * sh.k_mw(2:end-1))) ...
        ...
        + (T_m_sh(3:end) - T_m_sh(2:end-1)) * sh.k_mw(2:end-1) * sh.A_c_mw / sh.dx) ...
        ./ (sh.rho_mw(2:end-1) * sh.cp_mw(2:end-1) * sh.V_mw);
    dTdt(n.N_m_sh) = ((T_s_sh(end) - T_m_sh(end)) ./ (1 ./ (sh.h_s(end) * sh.A_s) + log(sh.r_c_sh / sh.r_i_sh) / (2 * pi * sh.dx * sh.k_mw(end))) ...
        ...
        + (T_m_sh(end-1) - T_m_sh(end)) * sh.k_mw(end) * sh.A_c_mw / sh.dx) ...
        ./ (sh.rho_mw(end) * sh.cp_mw(end) * sh.V_mw);
